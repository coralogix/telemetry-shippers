...
- The config enables:
  ...
  - a high-verbosity (`detailed`) debug exporter for both logs and metrics
...
- All AWS resource names include the `povilas` prefix for traceability

## SSH & manual verification
After `make deploy`, fetch the outputs:
...
- `systemd_check_command` – collector status check
- `journal_tail_command` – live log tail

Example session:

```bash
# SSH in (or copy the command from terraform output)
ssh -i ~/.ssh/id_rsa ubuntu@<public_ip>

# Inside the instance
sudo systemctl status otelcol-contrib.service --no-pager
sudo journalctl -u otelcol-contrib.service -n 50 --no-pager
sudo otelcol-contrib --version
```

Logs sent to the `debug` exporter are printed locally, making it easy to confirm end-to-end flow.
